...
def plot_individual_confusion_matrices(y_true, y_pred, labels, title_prefix, save_dir, epoch, theme='modern'):
    """
    为每个类别单独绘制混淆矩阵（修复版）
    ...
    Args:
        ...
        save_dir: 保存目录
        epoch: 当前epoch
        theme: 主题选择 ('modern', 'elegant', 'vibrant', 'academic', 'dark')
    """
    # 设置中文字体
    ...
    current_theme = themes.get(theme, themes['modern'])
    save_paths = []
    ...
    for i, label in enumerate(labels):
        ...
        # 绘制热力图 - 固定颜色条范围为0-100%
        im = sns.heatmap(cm, annot=True, fmt='d', cmap=current_theme['cmap'],
                        xticklabels=['负例', '正例'],
                        yticklabels=['负例', '正例'],
                        annot_kws={'size': 15, 'weight': 'bold', 'color': current_theme['annot_color']},
                        cbar_kws={'shrink': 0.8},
                        linewidths=2,
                        linecolor='white' if theme != 'dark' else '#34495e',
                        square=True,
                        vmin=0, vmax=max_value)  # 设置颜色映射范围

        # 设置颜色条样式 - 固定为0%-100%
        cbar = im.collections[0].colorbar
        cbar.ax.tick_params(labelsize=12, colors=current_theme['text_color'])
        
        # 固定颜色条刻度为0%-100%
        import matplotlib.ticker as ticker
        # 创建固定的刻度位置（基于最大值的百分比）
        tick_positions = [0, max_value * 0.25, max_value * 0.5, max_value * 0.75, max_value]
        tick_labels = ['0%', '25%', '50%', '75%', '100%']
        
        cbar.set_ticks(tick_positions)
        cbar.set_ticklabels(tick_labels)
        
        # 或者使用更简洁的方式 - 直接设置百分比格式
        # cbar.ax.yaxis.set_major_formatter(ticker.PercentFormatter(xmax=max_value, decimals=0))
        
        # 设置标题
        plt.title(f'{label} - {title_prefix} - Epoch {epoch}', 
                 fontsize=16, fontweight='bold', 
                 color=current_theme['title_color'], pad=15)
        ...
        # 设置刻度标签
        plt.tick_params(axis='both', labelsize=12, colors=current_theme['text_color'])
        ...
        save_path = os.path.join(save_dir, f'{label_name}_{title_prefix.lower().replace(" ", "_")}_epoch_{epoch}_{theme}.png')
        plt.savefig(save_path, dpi=300, bbox_inches='tight', 
                   facecolor=current_theme['bg_color'], edgecolor='none', 
                   format='png', transparent=False)
        plt.close()
        
        save_paths.append(save_path)
    
    print(f"   📊 已为{len(labels)}个类别生成单独的混淆矩阵")
...
def calculate_metrics_improved(labels, predictions, average='micro'):
    """
    改进的指标计算函数，更好地处理边界情况
    """
    ...
    # 计算基础指标，使用zero_division参数处理除零情况
    try:
        micro_f1 = f1_score(labels, predictions, average='micro', zero_division=0)
        macro_f1 = f1_score(labels, predictions, average='macro', zero_division=0)
        
        # 计算每个类别的F1分数，用于更详细的分析
        ...
        def hamming_accuracy(y_true, y_pred):
            """计算Hamming准确率（每个标签位置的准确率）"""
            return np.mean(y_true == y_pred)
        
        hamming_acc = hamming_accuracy(labels, predictions)
    ...
    return {
        ...
        'hamming_accuracy': hamming_acc * 100,
        'per_class_f1': per_class_f1.tolist()
    }


def analyze_class_distribution(y_true, labels):
    """
    分析类别分布，帮助理解不平衡问题
    """
    y_true = np.array(y_true)
    ...
    for i, label in enumerate(labels):
        ...
        positive_ratio = positive_count / len(y_true)
...
def train_model(config):
    ...
    # 创建混淆矩阵保存目录
    confusion_matrix_dir = os.path.join(config['results_dir'], 'confusion_matrices_qwen')
    ...
    # ==================== 新增：分析类别分布 ====================
    print("📊 分析训练集类别分布...")
    coarse_stats = analyze_class_distribution(
        [item[1] for item in train_dataset.data], 
        coarse_label_names
    )
    fine_stats = analyze_class_distribution(
        [item[2] for item in train_dataset.data], 
        fine_label_names
    )
    
    print("   粗粒度类别分布:")
    for label, stats in coarse_stats.items():
        print(f"   📊 {label}: {stats['positive_count']}/{stats['total_samples']} ({stats['positive_ratio']:.1%})")
    
    print("   细粒度类别分布:")
    for label, stats in fine_stats.items():
        print(f"   📊 {label}: {stats['positive_count']}/{stats['total_samples']} ({stats['positive_ratio']:.1%})")
    ...
    for epoch in range(config['epochs']):
        ...
        with torch.no_grad():
            for input_ids, attention_mask, coarse_labels, fine_labels, sent_ids, _, _, in val_progress:
                # 前向传播
                coarse_probs, fine_probs, load_balance_loss = model(input_ids, attention_mask)
        ...
        # ==================== 使用改进的指标计算 ====================
        val_coarse_metrics_micro = calculate_metrics_improved(all_coarse_labels, all_coarse_preds, average='micro')
        val_coarse_metrics_macro = calculate_metrics_improved(all_coarse_labels, all_coarse_preds, average='macro')
        val_constrained_fine_metrics_micro = calculate_metrics_improved(all_fine_labels, all_constrained_fine_preds, average='micro')
        val_constrained_fine_metrics_macro = calculate_metrics_improved(all_fine_labels, all_constrained_fine_preds, average='macro')
...
def main():
    ...
    parser.add_argument('--data_path', type=str, 
                       default='/mnt/cfs/huangzhiwei/NLP-WED0910/datas/train_merged_origin+qwen256.json',
                       help='训练数据路径')
    ...
    parser.add_argument('--lr', type=float, default=2.166285281378889e-05, help='学习率')
    parser.add_argument('--weight_decay', type=float, default=0.01, help='权重衰减')
    parser.add_argument('--epochs', type=int, default=40, help='训练轮数')
    parser.add_argument('--patience', type=int, default=5, help='早停耐心值')
    parser.add_argument('--threshold', type=float, default=0.3222194647688701, help='二分类阈值')
    ...
    # 保存路径
    parser.add_argument('--results_dir', type=str, default='training_results_0610', help='结果保存目录')
