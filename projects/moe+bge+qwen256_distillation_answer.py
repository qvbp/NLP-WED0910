...
class ErrorDetectionDataset(Dataset):
    def __init__(
        ...
        # 创建索引到标签名的反向映射
        self.coarse_idx_to_label = {v: k for k, v in coarse_labels.items()}
        self.fine_idx_to_label = {v: k for k, v in fine_labels.items()}
        self._get_data()
    ...
    def get_coarse_idx_to_label(self):
        return self.coarse_idx_to_label
    
    def get_fine_idx_to_label(self):
        return self.fine_idx_to_label
...
class ErrorDetectionDataLoader:
    ...
    def collate_fn(self, data):
        ...
        return input_ids, attention_mask, coarse_labels, fine_labels, sent_ids, coarse_soft_labels, fine_soft_labels, sents
...
def convert_predictions_to_labels(predictions, idx_to_label_map):
    """将预测的二进制向量转换为标签名称列表"""
    labels = []
    for pred in predictions:
        ...
        for idx, value in enumerate(pred):
            if value == 1:
                sample_labels.append(idx_to_label_map[idx])
        labels.append(sample_labels)
    return labels


def save_validation_results(val_results, save_path):
    """保存验证结果到JSON文件"""
...
def train_model(config):
    ...
    # 获取标签映射关系
    coarse_idx_to_label = val_dataset.get_coarse_idx_to_label()
    fine_idx_to_label = val_dataset.get_fine_idx_to_label()
    ...
    for epoch in range(config['epochs']):
        ...
        for batch_data in train_progress:
            # 解包数据（现在包含软标签）
            input_ids, attention_mask, coarse_labels, fine_labels, sent_ids, coarse_soft_labels, fine_soft_labels, _ = batch_data
        ...
        with torch.no_grad():
            for input_ids, attention_mask, coarse_labels, fine_labels, sent_ids, _, _, sents in val_progress:
                # 前向传播
                coarse_probs, fine_probs, load_balance_loss = model(input_ids, attention_mask)
                ...
                # 收集每个样本的详细结果
                batch_coarse_preds = coarse_preds.cpu().numpy()
                batch_fine_preds = constrained_fine_preds.cpu().numpy()
                batch_coarse_labels = coarse_labels.cpu().numpy()
                batch_fine_labels = fine_labels.cpu().numpy()
                
                for i in range(len(sent_ids)):
                    # 转换预测结果为标签名称
                    pred_coarse_labels = convert_predictions_to_labels([batch_coarse_preds[i]], coarse_idx_to_label)[0]
                    pred_fine_labels = convert_predictions_to_labels([batch_fine_preds[i]], fine_idx_to_label)[0]
                    
                    # 转换真实标签为标签名称
                    true_coarse_labels = convert_predictions_to_labels([batch_coarse_labels[i]], coarse_idx_to_label)[0]
                    true_fine_labels = convert_predictions_to_labels([batch_fine_labels[i]], fine_idx_to_label)[0]
                    
                    sample_result = {
                        "sent_id": int(sent_ids[i]),
                        "sent": sents[i],
                        "true_CourseGrainedErrorType": true_coarse_labels,
                        "true_FineGrainedErrorType": true_fine_labels,
                        "pred_CourseGrainedErrorType": pred_coarse_labels,
                        "pred_FineGrainedErrorType": pred_fine_labels,
                        "coarse_probs": coarse_probs[i].cpu().numpy().tolist(),
                        "fine_probs": fine_probs[i].cpu().numpy().tolist()
                    }
        ...
        if hierarchical_final_micro_f1 > best_val_f1:
            ...
            best_val_results_path = os.path.join(config['results_dir'], 'best_model_validation_results_qwe256+bge.json')
            save_validation_results(validation_results, best_val_results_path)
            print(f"   💾 最佳验证结果已保存到 {best_val_results_path}")
            print(f"   🎉 新的最佳模型! 已保存到 {model_save_path}")
        else:
            patience_counter += 1
        ...
        # 保存当前epoch的验证结果
        current_val_results_path = os.path.join(config['results_dir'], f'validation_results_epoch_{epoch+1}.json')
        save_validation_results(validation_results, current_val_results_path)
        print(f"   💾 当前验证结果已保存到 {current_val_results_path}")
