...
class ErrorDetectionDataLoader:
    ...
    def collate_fn(self, data):
        ...
        return input_ids, attention_mask, coarse_labels, fine_labels, sent_ids, sents
...
def train_model(config):
    ...
    for epoch in range(config['epochs']):
        ...
        for input_ids, attention_mask, coarse_labels, fine_labels, sent_ids, sents in train_progress:
            optimizer.zero_grad()
        ...
        with torch.no_grad():
            for input_ids, attention_mask, coarse_labels, fine_labels, sent_ids, sents in val_progress:
                # 前向传播
                coarse_probs, fine_probs, load_balance_loss = model(input_ids, attention_mask)
        ...
        if hierarchical_final_micro_f1 > best_val_f1:
            ...
            # 保存最佳epoch的验证结果
            best_val_results_path = os.path.join(config['results_dir'], 'best_validation_results.json')
    ...
    print(f"   - 最佳验证结果: {os.path.join(config['results_dir'], 'best_validation_results.json')}")
    print(f"   - 各epoch验证结果: {config['results_dir']}/validation_results_epoch_*.json")
