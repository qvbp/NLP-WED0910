...
class HierarchicalErrorClassifier(nn.Module):
    def __init__(
        ...
        num_fine_labels=14, 
        dropout=0.2,
        loss_weight_strategy='learnable_simple',  # 新增参数
        initial_coarse_weight=1.0,                # 粗粒度loss初始权重
        initial_fine_weight=1.0                   # 细粒度loss初始权重
    ):
        super().__init__()
        ...
        self._init_loss_weights(initial_coarse_weight, initial_fine_weight)
    
    def _init_loss_weights(self, initial_coarse_weight, initial_fine_weight):
        """初始化loss权重参数"""
        if self.loss_weight_strategy == 'fixed':
            # 固定权重，不参与训练
            self.register_buffer('coarse_weight', torch.tensor(initial_coarse_weight))
            self.register_buffer('fine_weight', torch.tensor(initial_fine_weight))
            
        elif self.loss_weight_strategy == 'learnable_simple':
            # 简单的可学习权重，无约束
            self.coarse_weight = nn.Parameter(torch.tensor(initial_coarse_weight))
            self.fine_weight = nn.Parameter(torch.tensor(initial_fine_weight))
            
        elif self.loss_weight_strategy == 'learnable_softmax':
            # 使用softmax归一化的权重，确保权重和为1
            initial_logits = torch.tensor([initial_coarse_weight, initial_fine_weight])
            self.loss_weight_logits = nn.Parameter(initial_logits)
            
        elif self.loss_weight_strategy == 'learnable_sigmoid':
            # 使用sigmoid的权重，确保权重在0-1之间
            self.coarse_weight_logit = nn.Parameter(torch.tensor(0.0))  # sigmoid(0) = 0.5
            self.fine_weight_logit = nn.Parameter(torch.tensor(0.0))
            
        elif self.loss_weight_strategy == 'uncertainty_based':
            # 基于不确定性的自适应权重（Multi-Task Learning Using Uncertainty）
            # log(sigma^2) 参数，sigma^2是任务的同方差不确定性
            self.log_var_coarse = nn.Parameter(torch.tensor(0.0))
            self.log_var_fine = nn.Parameter(torch.tensor(0.0))
            
        else:
            raise ValueError(f"不支持的loss权重策略: {self.loss_weight_strategy}")
    
    def get_loss_weights(self):
        """获取当前的loss权重"""
        if self.loss_weight_strategy == 'fixed':
            return self.coarse_weight, self.fine_weight
            
        elif self.loss_weight_strategy == 'learnable_simple':
            # 使用绝对值确保权重为正
            return torch.abs(self.coarse_weight), torch.abs(self.fine_weight)
            
        elif self.loss_weight_strategy == 'learnable_softmax':
            # softmax归一化
            weights = torch.softmax(self.loss_weight_logits, dim=0)
            return weights[0], weights[1]
            
        elif self.loss_weight_strategy == 'learnable_sigmoid':
            # sigmoid映射到0-1之间
            coarse_w = torch.sigmoid(self.coarse_weight_logit)
            fine_w = torch.sigmoid(self.fine_weight_logit)
            return coarse_w, fine_w
            
        elif self.loss_weight_strategy == 'uncertainty_based':
            # 基于不确定性的权重: 1/(2*sigma^2)
            # 不确定性越大，权重越小
            precision_coarse = torch.exp(-self.log_var_coarse)
            precision_fine = torch.exp(-self.log_var_fine)
            return precision_coarse / 2.0, precision_fine / 2.0
    
    def calculate_weighted_loss(self, coarse_loss, fine_loss):
        """计算加权loss"""
        coarse_weight, fine_weight = self.get_loss_weights()
        
        if self.loss_weight_strategy == 'uncertainty_based':
            # 基于不确定性的loss包含正则化项
            weighted_loss = (coarse_weight * coarse_loss + self.log_var_coarse / 2.0 + 
                           fine_weight * fine_loss + self.log_var_fine / 2.0)
        else:
            weighted_loss = coarse_weight * coarse_loss + fine_weight * fine_loss
            
        return weighted_loss, coarse_weight, fine_weight
...
def train_single_config(configs, param_combination_desc=""):
    ...
    model = HierarchicalErrorClassifier(
        ...
        loss_weight_strategy=getattr(configs, 'loss_weight_strategy', 'learnable_simple'),
        initial_coarse_weight=getattr(configs, 'initial_coarse_weight', 1.0),
        initial_fine_weight=getattr(configs, 'initial_fine_weight', 1.0)
    ).to(configs.device)
    ...
    for epoch in range(configs.epochs):
        ...
        with tqdm(
            ...
            for input_ids, attention_mask, coarse_labels, fine_labels, sent_ids in pbar:
                ...
                loss, coarse_weight, fine_weight = model.calculate_weighted_loss(coarse_loss, fine_loss)
                ...
                # 更新进度条，显示权重信息
                if hasattr(model, 'loss_weight_strategy') and model.loss_weight_strategy != 'fixed':
                    pbar.set_postfix(
                        loss=f'{loss.item():.3f}',
                        cw=f'{coarse_weight.item():.3f}',
                        fw=f'{fine_weight.item():.3f}'
                    )
                else:
        ...
        with torch.no_grad():
            for input_ids, attention_mask, coarse_labels, fine_labels, sent_ids in val_dataloader:
                ...
                coarse_loss = criterion(coarse_probs, coarse_labels)
                fine_loss = criterion(fine_probs, fine_labels)
                loss, _, _ = model.calculate_weighted_loss(coarse_loss, fine_loss)
                val_loss += loss.item()
...
def grid_search_hyperparameters(args):
    ...
    # 从模型路径提取模型名称，用于区分不同模型
    model_short_name = get_model_name_from_path(args.model_path)
    ...
    param_grid = {
        ...
        'seed': [42, 3407],
        'loss_weight_strategy': ['fixed', 'learnable_simple', 'learnable_softmax', 'uncertainty_based']
    }
    ...
    base_config = {
        ...
        'epochs': 40,
        'patience': 5,
        'data_path': "../datas/train.json",
        'val_data_path': "../datas/val.json",
        'checkpoint_dir': os.path.join('grid_search_checkpoints', model_short_name),
        'weight_decay': 0.01,
        'initial_coarse_weight': 1.0,
        'initial_fine_weight': 1.0
    }
    ...
    # 创建结果保存目录（包含模型名称）
    results_dir = os.path.join('grid_search_results', model_short_name)
    ...
    for i, param_combination in enumerate(param_combinations):
        print(f"\n{'='*80}")
        print(f"[{model_short_name}] 测试组合 {i+1}/{len(param_combinations)}")
    ...
    # 打印最终结果
    print(f"\n{'='*80}")
    print(f"[{model_short_name}] 网格搜索完成!")
...
def analyze_grid_search_results(model_name, results_file=None):
    """分析网格搜索结果"""
    if results_file is None:
        results_file = os.path.join('grid_search_results', model_name, 'final_grid_search_results.json')
    ...
    print(f"\n[{model_name}] 网格搜索结果分析 (基于 {len(successful_results)} 个成功的实验)")
    print("="*80)
    ...
    for param_name in param_names:
        ...
        print(f"\n{param_name}:")
        # 对于loss_weight_strategy，提供更详细的说明
        if param_name == 'loss_weight_strategy':
            strategy_descriptions = {
                'fixed': '固定权重(1:1)',
                'learnable_simple': '简单可学习权重', 
                'learnable_softmax': 'softmax归一化权重',
                'learnable_sigmoid': 'sigmoid权重',
                'uncertainty_based': '基于不确定性的权重'
            }
            for value, performances in sorted(param_performance.items(), key=lambda x: np.mean(x[1]), reverse=True):
                avg_perf = np.mean(performances)
                std_perf = np.std(performances)
                desc = strategy_descriptions.get(value, value)
                print(f"  {value} ({desc}): {avg_perf:.2f}% ± {std_perf:.2f}% (n={len(performances)})")
        else:
    ...
    # 特别分析loss权重策略的效果
    if 'loss_weight_strategy' in param_names:
        print(f"\nLoss权重策略详细分析:")
        strategy_results = {}
        for result in successful_results:
            strategy = result['params']['loss_weight_strategy']
            if strategy not in strategy_results:
                strategy_results[strategy] = []
            strategy_results[strategy].append(result['metrics'])
        
        for strategy, results in strategy_results.items():
            print(f"\n{strategy}策略 (n={len(results)}):")
            avg_metrics = {}
            for metric_name in ['hierarchical_final_micro_f1', 'hierarchical_final_macro_f1']:
                values = [r[metric_name] for r in results]
                avg_metrics[metric_name] = np.mean(values)
                print(f"  {metric_name}: {np.mean(values):.2f}% ± {np.std(values):.2f}%")


def parse_arguments():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='网格搜索超参数调优')
    
    # 必需参数
    parser.add_argument('--model_path', type=str, required=True,
                        help='预训练模型路径 (例如: /path/to/bert-base-chinese)')
...
if __name__ == '__main__':
    ...
    # 运行网格搜索
    print(f"开始为模型 {model_name} 进行网格搜索超参数调优...")
    print(f"模型路径: {args.model_path}")
    print("调优的超参数: dropout, batch_size, lr, threshold, seed, loss_weight_strategy")
    print("Loss权重策略包括:")
    print("  - fixed: 固定权重(1:1)")
    print("  - learnable_simple: 简单可学习权重")  
    print("  - learnable_softmax: softmax归一化权重")
    print("  - uncertainty_based: 基于不确定性的自适应权重")
    print("将记录12个性能指标并保存到JSON文件中")
        
    # 开始网格搜索
    best_result, all_results = grid_search_hyperparameters(args)
    
    # 分析结果
    print(f"\n正在分析 {model_name} 的结果...")
    analyze_grid_search_results(model_name)